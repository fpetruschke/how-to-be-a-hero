...
(function registerTokensEffekteExport() {
  ...
  function zeichneEmoji(ctx, emoji, cx, cy, radius) {
    const fontSize = Math.max(12, Math.round(radius * 1.1));
  ...
  function berechneNameFontSizePx(tokenPx, nameFontSizePxEinstellung) {
    ...
    if (Number.isFinite(fest) && fest > 0) {
      return Math.min(28, Math.max(6, Math.round(fest)));
    }
    return Math.max(8, Math.min(13, Math.round(Number(tokenPx) * 0.11)));
  }
  ...
  function zeichneNamenMitRahmen(ctx, name, x, tokenY, tokenPx, _nameHoehePx, borderColor, borderWidthPx, optionen) {
    ...
    const effektLabel = opts.effektLabel === true;
    const overlapToken = effektLabel || opts.overlapToken !== false;
    const lw = Math.max(0, Number(borderWidthPx) || 0);
    const overlapPx = overlapToken
      ? Math.max(lw, Math.round(tokenPx * (effektLabel ? 0.2 : 0.14)))
      : 0;
    const hoehenVersatzPx = overlapToken ? Math.round(tokenPx * (effektLabel ? 0.06 : 0.03)) : 0;
    ...
    if (lw > 0) {
      ...
      ctx.lineWidth = lw;
      const inset = lw / 2;
      ctx.strokeRect(x + inset, boxTop + inset, boxWidth - lw, boxHeight - lw);
    }
  ...
  async function zeichneTokenZelle(ctx, item, x, y, tokenPx, einstellungen, bildCache) {
    const shape = item.shape || 'kreis';
    const borderColor = einstellungen.borderColor || '#000000';
    const borderWidthPx = Number(einstellungen.borderWidthPx) || 0;
    const showName = einstellungen.showName !== false;
    ...
    zeichneTokenRahmen(ctx, shape, x, y, tokenPx, borderColor, borderWidthPx);

    if (showName && item.name) {
      zeichneNamenMitRahmen(ctx, item.name, x, y, tokenPx, nameHoehePx, borderColor, borderWidthPx, {
        overlapToken: true,
  ...
  function berechneRaster(abmessungen, einstellungen, itemAnzahl, dpi) {
    ...
    const nameBereichAktiv = showName || effectLabels;
  ...
  function baueExportItems(einstellungen, alleEntitaeten) {
    ...
    const items = entitaeten.map((entitaet) => ({
      typ: 'token',
      ...entitaet,
  ...
  async function renderTokensSeite(ctx, items, seitenIndex, raster, einstellungen, bildCache) {
      ...
      if (item.typ === 'effekt') {
        ...
        if (
          ...
          zeichneNamenMitRahmen(
            ...
            Number(einstellungen.borderWidthPx) || 0,
            { overlapToken: true, effektLabel: true, nameFontSizePx: einstellungen.nameFontSizePx },
          );
        }
      } else {
        await zeichneTokenZelle(ctx, item, x, y, raster.tokenPx, einstellungen, bildCache);
      }
    }
  }
  ...
  async function renderTokensCanvas(einstellungen, optionen) {
    const opts = optionen && typeof optionen === 'object' ? optionen : {};
    const dpi = Number.isFinite(Number(opts.dpi)) ? Math.max(72, Number(opts.dpi)) : DEFAULT_DPI;
    ...
    const raster = berechneRaster(abmessungen, einstellungen, items.length, dpi);
    ...
    for (let s = 0; s < seitenAnzahl; s += 1) {
      ...
      await renderTokensSeite(ctx, items, s, raster, einstellungen, bildCache);
      seiten.push(canvas);
    }
    return {
      seiten,
