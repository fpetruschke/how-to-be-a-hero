...
window.HTBAH_KOMPONENTEN.InventarEditorPanel = {
  name: 'InventarEditorPanel',
  emits: ['change', 'remove'],
  props: {
    inventar: { type: Array, required: true },
  },
  data() {
    const SHARED = window.HTBAH_SHARED || {};
    return {
      inventarEditId: null,
      inventarBackup: null,
      inventarQuillNachId: {},
  ...
  methods: {
    ...
    inventarEintragFinden(id) {
      return this.inventar.find((e) => e.id === id);
    },
    inventarQuillZerstoeren(id) {
      ...
      this.$nextTick(() => {
        ...
        if (root && typeof root.querySelectorAll === 'function') {
          root.querySelectorAll('.ql-toolbar.ql-snow').forEach((node) => {
            node.remove();
          });
        }
      });
    ...
    inventarAbbrechenZeile() {
      ...
      if (eintrag && this.inventarIstInhaltLeer(eintrag)) {
        const index = this.inventar.indexOf(eintrag);
        if (index !== -1) {
    ...
    async inventarZeileLoeschen(eintrag) {
      ...
      const gegenstandId = eintrag.id;
      const index = this.inventar.indexOf(eintrag);
      if (index !== -1) {
        this.inventar.splice(index, 1);
      }
      this.$emit('remove', { gegenstandId });
      this.$emit('change');
    },
  ...
  beforeUnmount() {
    ...
    this.inventarQuillRefFnCache = Object.create(null);
  },
  template: `
          <div class="inventar-editor-panel">
